...
public class BinaryTreeMatrix {
    ...
    public BinaryTreeMatrix(int[][] res) {
        ...
        root = constructor(res, pq);
    }

    public static MatNode constructor(int[][] res, PriorityQueue<int[]> pq) {

        boolean[] parent = new boolean[res.length];
        MatNode[] nodeArr = new MatNode[res.length];
        ...
        while (!pq.isEmpty()) {
            ...
            if (arr[0] != 0) {
                for (int i = 0; i < res.length; i++) {
                    if (!parent[i] && res[arr[1]][i] != 0) {
                        if (nodeArr[arr[1]].left != null)
                            nodeArr[arr[1]].left = nodeArr[i];
                        else
                            nodeArr[arr[1]].right = nodeArr[i];
                        parent[i] = true;
                    }
                }
                if (pq.size() == res.length - 1)
                    main_root = nodeArr[arr[1]];
            }
        }
    ...
    private static void display(MatNode node) {
        if (node == null)
            return;
    ...
    public static void main(String[] args) {
        ...
        BinaryTreeMatrix matree = new BinaryTreeMatrix(res);
        matree.display();
