...
public class VerticalOrder {
    ...
    public static ArrayList<ArrayList<Integer>> verticalWeightedOrder(BinaryTree.Node node) {
        ...
        while (!pq.isEmpty()) {
            ...
            while (size-- > 0) {
                ...
                res.get(rp.level).add(rp.node.data);
                if (rp.node.left != null)
                    pq.add(new Pair(rp.node.left, rp.level - 1));
                if (rp.node.right != null)
                    pq.add(new Pair(rp.node.right, rp.level + 1));
            }
        }
        return res;
    }
    ...
    public static void main(String[] args) {
        ...
        int[] preOrder = { 120, 110, 70, -1, -1, 80, 50, -1, -1, 90, -1, -1, 30, 60, 100, -1, -1, -1, 40, 20, -1, -1,
                10, -1, -1 };
        BinaryTree tree = new BinaryTree(preOrder);
        tree.display();
        System.out.println(verticalOrder(tree.root));
        System.out.println(verticalWeightedOrder(tree.root));
    }
