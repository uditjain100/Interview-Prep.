...
public class BinaryTreeConstruct {
    ...
    public BinaryTreeConstruct(int[] order1, int[] order2, int i) {
        if (i == 1)
            root = constructorUsingPreAndIn(order1, 0, order1.length - 1, order2, 0, order2.length - 1);
        if (i == 2)
            root = constructorUsingPostAndIn(order1, 0, order1.length - 1, order2, 0, order2.length - 1);
        if (i == 3)
            root = constructorUsingPreAndPost(order1, 0, order1.length - 1, order2, 0, order2.length - 1);
    }
    ...
    public static Node constructorUsingPostAndIn(int[] post, int psi, int pei, int[] in, int isi, int iei) {
        ...
        nn.left = constructorUsingPostAndIn(post, psi, psi + totalElements - 1, in, isi, idx - 1);
        nn.right = constructorUsingPostAndIn(post, psi + totalElements, pei - 1, in, idx + 1, iei);
        return nn;
    }

    public static Node constructorUsingPreAndPost(int[] pre, int prsi, int prei, int[] post, int posi, int poei) {

        if (posi > poei || prsi > prei)
            return null;
        if (posi == poei)
            return new Node(post[posi]);
        Node nn = new Node(pre[prsi]);
        ...
        nn.left = constructorUsingPreAndPost(pre, prsi + 1, prsi + totalNoOfElements, post, posi, idx);
        nn.right = constructorUsingPreAndPost(pre, prsi + totalNoOfElements + 1, prei, post, idx + 1, poei - 1);
    ...
    public static void main(String[] args) {
        ...
        BinaryTreeConstruct btree2 = new BinaryTreeConstruct(pre2, in2, 1);
        btree2.display();
        BinaryTreeConstruct btree3 = new BinaryTreeConstruct(post2, in2, 2);
        btree3.display();
        BinaryTreeConstruct btree4 = new BinaryTreeConstruct(pre2, post2, 3);
        btree4.display();
