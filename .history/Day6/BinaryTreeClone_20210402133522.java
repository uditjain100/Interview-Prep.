...
public class BinaryTreeClone {
    ...
    public static void randomPointerWork(TreeNode node) {
        ...
        ArrayList<BinaryTree.Node> list = new ArrayList<>(map.keySet());
        node.random = map.get(list.get(rand.nextInt(list.size())));
    ...
    private static void display(TreeNode node) {
        ...
        String str = "[" + node.data + "," + node.random.data + "] => ";

        if (node.left != null)
            str += "[" + node.left.data + "," + node.random.data + "] , ";
        if (node.right != null)
            str += "[" + node.right.data + "," + node.random.data + "] , ";
