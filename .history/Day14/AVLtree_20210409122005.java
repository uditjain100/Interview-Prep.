public class AVLtree {
    ...
    public static Node remove(Node node, Node parent, int data, boolean isLeft) {
        if (node == null)
            return null;

        if (node.data < data) {
            node.right = remove(node.right, node, data, false);
        } else if (node.data > data) {
            node.left = remove(node.left, node, data, true);
        } else {
