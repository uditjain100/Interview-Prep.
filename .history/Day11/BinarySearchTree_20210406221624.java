public class BinarySearchTree {
    ...
    public static int height(int[] preOrder) {
        return height(preOrder, Integer.MIN_VALUE, Integer.MAX_VALUE);
    }

    public static int height(int[] preOrder, int lb, int ub) {
        if (idx == preOrder.length || preOrder[idx] < lb || preOrder[idx] > ub)
            return 0;

        idx++;
        int lr = constructor(preOrder, lb, preOrder[idx - 1]);
        int rr = constructor(preOrder, preOrder[idx - 1], ub);
    ...
    public static void main(String[] args) {
        ...
        tree.display();
        System.out.println(tree.height(pre));
        tree.add(2);
        tree.add(20);
        tree.remove(5);
        tree.display();
