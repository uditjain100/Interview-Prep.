...
public class K_Concatenation {
    ...
    public static int fun(int[] arr, int k) {
        ...
        if (k == 1)
            return (KadanesAlgo_Linear(arr) % mod);
        if (sum < 0)
            return (KadanesAlgo_Circular(arr) % mod);
        int res = (int) (KadanesAlgo_Circular(arr) % mod);
        int r = (((k - 2) * sum) % mod);
        return (res + r) % mod;
    }
    ...
    public static int KadanesAlgo_Linear(int[] arr) {
        ...
        for (int ele : arr) {
            curr_max_sum = Math.max(ele, ele + curr_max_sum) % mod;
            max_sum = Math.max(curr_max_sum, max_sum) % mod;
        }
        return max_sum;
    }
